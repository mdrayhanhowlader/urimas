...
foreach ($books_raw_data as $item) {
    if (is_numeric($item)) {
        $bid = (int)$item;
        if ($bid > 0) $item_pairs[] = ['id' => $bid, 'variant' => '', 'color' => ''];
    } elseif (is_array($item) && isset($item['id'])) {
        $bid = (int)$item['id'];
        if ($bid > 0) $item_pairs[] = ['id' => $bid, 'variant' => sanitize($item['variant'] ?? ''), 'color' => sanitize($item['color'] ?? '')];
    }
}
...
try {
    ...
    $stmt = $pdo->prepare("
        INSERT INTO orders (name, phone, email, address, area, books_data, transaction_id, total, status)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'pending')
    ");
    $stmt->execute([$name, $phone, $email, $address, $area, $books_json, $transaction_id ?: '', $total]);
...
function sendEmailNotification($settings, $order_id, $name, $phone, $address, $area, $books, $payment_method, $txn, $delivery, $total) {
    ...
    $book_lines = '';
    foreach ($books as $b) {
        $v = !empty($b['variant']) ? " ({$b['variant']})" : '';
        $book_lines .= "  • {$b['name']}{$v} — ৳{$b['price']}\n";
    }
